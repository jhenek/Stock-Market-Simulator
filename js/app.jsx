	...
	class SignUp extends React.Component {
		...
		handleLogin = (event) => {
			this.setState({
				...
				data: {...this.state.data, login: event.currentTarget.value
			}});
		};
		...
		handlePassword = (event) => {
			this.setState({
				...
				data: {...this.state.data, password: event.currentTarget.value}
			});
		};
		...
		handleFirstName =  (event) => {
			this.setState({
				...
				data: {...this.state.data, firstName: event.currentTarget.value}
			});
		};
		...
		handleLastName =  (event) => {
			this.setState({
				...
				data: {...this.state.data, lastName: event.currentTarget.value}
			});
		};
		...
		handleBallance = (event) => {
			this.setState({
				...
				data: {...this.state.data, ballance: event.currentTarget.value}
			});
		};
		...
		handleFp = (event) => {
			this.setState({
				...
				data: {...this.state.data, fp: event.currentTarget.value}
			});
		};
		...
		handleFpl = (event) => {
			this.setState({
				...
				data: {...this.state.data, fpl: event.currentTarget.value}
			});
		};
		...
		handlePgb = (event) => {
			this.setState({
				...
				data: {...this.state.data, pgb: event.currentTarget.value}
			});
		};
		...
		handleFpc = (event) => {
			this.setState({
				...
				data: {...this.state.data, fpc: event.currentTarget.value}
			});
		};
		...
		handleFpa = (event) => {
			this.setState({
				...
				data: {...this.state.data, fpa: event.currentTarget.value}
			});
		};
		...
		handleDl24 = (event) => {
			this.setState({
				...
				data: {...this.state.data, dl24: event.currentTarget.value}
			});
		};
		...
		handleSignUp = (event) => {
			...
			if (status) {
				...
				localStorage.setItem(this.state.login, JSON.stringify(this.state.data));
                  ...
                  <Link to="/" className="header">Sign in</Link>
					...
					<main className="mainSignup">
						...
						<form className="form">
							...
							<div className={(this.state.showAlert && (this).checkLogin()) ? "alert" : "alert nonActive"}>!    Login already in use</div>
						</form>
						<form className="form">
						...
						<button onClick={this.handleSignUp} className="button">Create new account</button>
					</main>
				</div>
			)
		}
	...
	class Main extends React.Component {
		...
		componentDidMount() {
			...
			this.setState({
				...
				ballance: parseFloat(user.ballance).toFixed(2),
				FP: parseInt(user.fp),
			...
			return (
				<div className="container">
					<header className="mainHeader">
						<h2 className="mainHeaderTitle">Stocks</h2>
			...
			return(
				<section className="buySell">
					...
					<div className="buttons">
						<button onClick={this.handleClickCancel} className="buttonBuySell">Cancel</button>
						<button onClick={this.handleClickBuySell} className="buttonBuySell">{this.props.data.type}</button>
						{/* <input className="buttonBuySell" type="submit" value="Buy" /> */}
					</div>
				</section>
			)
		}
